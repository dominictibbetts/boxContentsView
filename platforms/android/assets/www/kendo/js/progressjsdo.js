...
/*
 * progress.util.js Contains support objects used by the jsdo and/or session object
 */
...
progress.data.LocalStorage = function LocalStorage( ) {
    ...
    if (typeof(localStorage) == "undefined") {
        ...
        throw new Error("Error: No support for localStorage");
    }
...
//@ sourceURL=progress.jsdo.3.1.js
...
//          localServiceName = serviceName;
    ...
    this._convertType = function (value, type, itemType) {
        ...
        try {
            if (type == 'array') {
                ...
                // PSC00319759 First strip out '[' and ']' from string
    ...
    */
    ...
    // The request param should be defined for saveChanges(), but not needed when storing data to local storage
    ...
    this._deleteProdsProperties = function (record, clearErrorString) {
        ...
        if (record) {
            ...
            if (clearErrorString && record._errorString) {
                record._errorString = undefined;    
            }
        }
    };
                ...
                if (keyFields.length == 0 || addMode == progress.data.JSDO.MODE_EMPTY) {
                    ...
                    for (var i=0; i < jsonObject.length; i++) {
                        ...
                        if (match) {
                            ...
                            {       
                                // Only throw exception if the existing bi data is not the same as the new bi data
                                ...
                                if (beforeRecord) {
                                    var beforeObject = 
                                        this._getBeforeRecordFromObject(jsonObject[i], beforeImageJsonObject, tableName);
                                    if (beforeObject)
    ...
    this._onReadyStateChangeGeneric = function () {
        ...
        if (xhr.readyState == 4) {
            ...
            if ((typeof xhr.onResponseProcessedFn) == 'function') {
                ...
                xhr.onResponseProcessedFn(xhr.pdsession, result, errorObject);
            }           
        }
    };
    ...
    */
    function doFormLogin ( args ){
        ...
        if ( contentType && contentType.indexOf("application/json") >= 0 ) {
            ...
            if (theSession.loginHttpStatus == 200 ) {  // need to do only 200 for async to work with MWA down
    ...
    this._loginComplete = function ( pdsession, result, errObj ) {
        pdsession.trigger("afterLogin", pdsession, result, errObj );
    };
    ...
    this.logout = function () {
        ...
        if ( arguments.length > 0) {
            if (typeof(arguments[0]) === 'object') {
                ...
                if ( isAsync && (typeof isAsync != 'boolean') ) {
                    throw new Error(jsdoMSG033, 'Session', 'logout', 'The async argument was invalid.');
                }
            }
        }
        ...
        if (isAsync && this.authenticationModel === progress.data.Session.AUTH_TYPE_ANON ) {
            ...
            this._logoutComplete(this, null, errorObject);
        }
    ...
    this.addCatalog = function (  ) {
        ...
        if ( arguments.length > 0) {
            if (typeof(arguments[0]) === 'object') {
                ...
                if ( !catalogURI || (typeof catalogURI != 'string') ) {
                    throw new Error(jsdoMSG033, 'Session', 'addCatalog', 'The catalogURI argument was missing or invalid.');
                }
                catalogUserName = arguments[0].userName;
                if ( catalogUserName && (typeof catalogUserName != 'string') ) {
                    throw new Error(jsdoMSG033, 'Session', 'addCatalog', 'The catalogUserName argument was invalid.');
                }
                catalogPassword = arguments[0].password;
                if ( catalogPassword && (typeof catalogPassword != 'string') ) {
                    throw new Error(jsdoMSG033, 'Session', 'addCatalog', 'The catalogPassword argument was invalid.');
                }
                isAsync = arguments[0].async;
                if ( isAsync && (typeof isAsync != 'boolean') ) {
                    throw new Error(jsdoMSG033, 'Session', 'addCatalog', 'The async argument was invalid.');
                }
            }
            else {
        ...
        if (progress.data.ServicesManager.getSession(catalogURI) != undefined) {
            if (isAsync) {
                ...
                setTimeout( this._addCatalogComplete, 10, this, progress.data.Session.CATALOG_ALREADY_LOADED, null );
                return progress.data.Session.ASYNC_PENDING;
            }
    ...
    this._addCatalogComplete = function( pdsession, result, errObj ) {
        pdsession.trigger("afterAddCatalog", pdsession, result, errObj);
    };
...
progress.data.Session.prototype._eventNames = ["offline", "online", "afterLogin", "afterAddCatalog", "afterLogout"];  // events supported by Session
